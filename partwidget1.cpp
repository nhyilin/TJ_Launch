...
#include "ui_partwidget1.h"

PartWidget1::PartWidget1(QWidget *parent)
    : IWidget(parent)
    , ui(new Ui::PartWidget1)
{
    ui->setupUi(this);
    // 读取配置文件
    loadConfig();
...
void PartWidget1::startAnimation() {
    if (!showW) return;  // 确保 showW 存在

    // **1. 设置 showW 初始状态**
    QRect originalPos = showW->geometry();
    QRect startPos = originalPos.translated(0, -50);  // **初始位置上移**
